...
def transit_logic_v1_1(
    ...
    while total_number_of_vehicles_dropped < NUMBER_OF_METRO_DROP_OFF:
        ...
        for mv_index, moving_vehicles in enumerate(moving_vehicles_platform):
            if moving_vehicles.can_input():
                # 启用转运车在喷砂间和总成分离的模式
                if USE_OF_THE_BLASTING_ROOM and ENABLING_SEPARATION_OF_TRANSFER_VEHICLES:
                    for platform_ in platform_list:
                        if platform_.can_output() and platform_.name in STATIONS_SERVICED_BY_TRANSFER_VANS[mv_index]:
                            for platform__ in platform_list:
                                if platform__.can_input() and platform__.type in ALL_STAGE_CONFIG[platform_.type]["下一台位类型"] and platform_.metro.type in ALL_STAGE_CONFIG[platform__.type]["适用车型"] and not platform__ in reservations_list:
